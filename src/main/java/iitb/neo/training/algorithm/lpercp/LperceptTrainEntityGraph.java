...
public class LperceptTrainEntityGraph {
	...
	public static void train(String dir, Random r, int numIterations,
			double regularizer, boolean finalAvg, boolean ignoreConfusion,
			String mappingFile) throws IOException {
		Model model = new Model();
		model.read(dir + File.separatorChar + "model");

		LocalAveragedPerceptronEntityGraph lpton = new LocalAveragedPerceptronEntityGraph(
				model, r, numIterations, regularizer, finalAvg, mappingFile);

		Dataset<EntityGraph> train = null;

		train = (Dataset<EntityGraph>) new EntityGraphMemoryDataset(dir
				+ File.separatorChar + "train");

		System.out.println("starting training with regularizer = "
				+ regularizer + ", iterations = " + numIterations
				+ ", finalAvg = " + finalAvg + ", ignoreConfusion = "
				+ ignoreConfusion);

		long start = System.currentTimeMillis();
		Parameters params = lpton.train(train);
		long end = System.currentTimeMillis();
		System.out.println("training time " + (end - start) / 1000.0
				+ " seconds");
	...
	public static void train(String dir) throws IOException {
		throw new NotImplementedException(
				"Insufficient parameters to start training");

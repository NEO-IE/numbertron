...
public class GeneratePRCurves {
	...
	public void generate() throws Exception {
		String outputFile = prCurveFilePrefix
				+ modelName.substring(modelName.lastIndexOf("/") + 1) + ".tsv";
		...
		for (double conf = startConf; conf <= endConf; conf += confwidth) {
			...
			emodel.evaluate();
			pw.write(conf + "\t" + emodel.res.precision + "\t"
					+ emodel.res.recall + "\t"
					+ f(emodel.res.precision, emodel.res.recall) + "\n");
		}

...
public class UnitLocationSpotting extends Spotting {
	...
	public UnitLocationSpotting(String corpusPath, CorpusInformationSpecification cis,
			RuleBasedDriver rbased, String countriesListFileName) {
		...
		try {
			...
			BufferedReader br = new BufferedReader(
					new FileReader(countriesListFileName));
		...
		while (di.hasNext()) {
			...
			for (CoreMap sentence : sentences) {
				...
				try {
					...
					//include NAs, around 25% of the instances should be NAs
					List<Relation> currRels = rbased.spotPossibleRelations(deps, sentence, true, 0.25);

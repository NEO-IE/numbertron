...
public class KeywordFeatureGenerator implements FeatureGenerator {
	...
	private static HashSet<String> deltaWords;
	static
	{
		...
		String[] fixedDeltaWords = new String[]
				{"change", "up", "down", "males", "females", "male", "female",
				"growth", "increase", "decrease", "decreased", "increased", 
				"changed", "grown", "grew", "surge", "surged", "rose"};
		deltaWords = new HashSet<>();
		for(String delta: fixedDeltaWords){
			deltaWords.add(StemUtils.getStemWord(delta));
		}
	...
	public List<String> generateFeatures(Integer arg1StartOffset,
		...
		if(features.size() > 0){  //atleast one keyword feature fired.
			features.add("meta: keywords");
		}
	...
	public  List<String> getDeltaFeatures( String[] tokens, 
			String[] postags,
		...
		if (lcaUp + lcaDown < 12) {
			...
			for (int i=0; i < lcaUp; i++) {
				 ...
				 */
				String stemToken = StemUtils.getStemWord(tokens[path1[i]]);
				if(i > 0 && deltaWords.contains(stemToken)){
					features.add("meta: deltawords");
				}	
			}
			for (int j=0; j < lcaDown; j++) {
				 ...
				 * adding keywords as features
				 */
				String stemToken = StemUtils.getStemWord(tokens[path2[lcaDown-j]]);
				if(lcaUp + j > 0  && deltaWords.contains(stemToken)){
					features.add("meta: deltawords");
				}
			}
		}	

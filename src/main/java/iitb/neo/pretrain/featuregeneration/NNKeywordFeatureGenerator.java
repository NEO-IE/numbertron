...
public class NNKeywordFeatureGenerator implements FeatureGenerator {
		...
		if (lcaUp + lcaDown < 12) {
			...
			for (int i=0; i < lcaUp; i++) {
				...
				if(i > 0 && postags[path1[i]].equals("NN")){
					features.add("key: "+ StemUtils.getStemWord(tokens[path1[i]]));
				}	
			}
			for (int j=0; j < lcaDown; j++) {
				...
				if(lcaUp + j > 0  && postags[path2[lcaDown-j]].equals("NN")){
					features.add("key: " + StemUtils.getStemWord(tokens[path2[lcaDown-j]]));
				}
			}
		}	
		return features;
